...
> 在 ComfyUI 中学习 Pose ControlNet。使用 OpenPose 骨骼图控制角色姿态，并通过两次生成的工作流创建大尺寸图像。
...
<img src="/img/tutorial/controlnet/openpose_example.jpg" alt="OpenPose 示例" width="2048" height="512" data-path="images/tutorial/controlnet/openpose_example.jpg" />
...
![ComfyUI 工作流 - Pose ControlNet](/img/external/raw-githubusercontent-com/controlnet/pose_controlnet_2_pass.png)

<Tip>
  元数据中包含工作流 JSON 的图片可直接拖入 ComfyUI，或使用菜单 `Workflows` -> `Open (ctrl+o)` 来加载工作流。
  该图片已包含对应模型的下载链接，直接拖入 ComfyUI 将会自动提示下载。
</Tip>

请下载下面的图片，我们将把它作为输入：

![ComfyUI Pose 输入图片](/img/external/raw-githubusercontent-com/controlnet/pose_controlnet_2_pass_input.png)

### 2. 手动安装模型

<Note>
  如果你的网络无法顺利完成对应模型的自动下载，请尝试手动下载下面的模型，并放置到指定目录：
</Note>

* [control\_v11p\_sd15\_openpose\_fp16.safetensors](https://huggingface.co/comfyanonymous/ControlNet-v1-1_fp16_safetensors/blob/main/control_v11p_sd15_openpose_fp16.safetensors?download=true)
...
<img src="/img/tutorial/controlnet/flow_diagram_pose_controlnet_2_pass.jpg" alt="ComfyUI 工作流 - Pose ControlNet 流程图" width="1803" height="1081" data-path="images/tutorial/controlnet/flow_diagram_pose_controlnet_2_pass.jpg" />
...
4. 点击 `Load Image` 节点中的选择按钮，上传之前提供的姿态输入图片，或使用你自己的 OpenPose 骨骼图。
5. 确保 `Load Checkpoint` 可以加载 **japaneseStyleRealistic\_v20.safetensors**。
6. 点击 `Queue` 按钮，或使用快捷键 `Ctrl(cmd) + Enter` 来执行图像生成。
